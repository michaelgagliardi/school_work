...
{
    char *argv[MAXARGS];
    int bg,bin;             //return values of builtin and parseline
    pid_t pid;              //pid of the created process
    sigset_t mask;          //signal set for blocking signals

    bg = parseline(cmdline, argv); //sets bg to return value of parseline
    bin = builtin_cmd(argv,cmdline);            //sets bin to return value of builtin

    if(!bin){                            //if not a built in command
        sigemptyset(&mask);              //initializing the signal set pointed to by mask
        sigaddset(&mask, SIGCHLD);      //adds sigchld signal to mask signal set
        sigprocmask(SIG_BLOCK, &mask, NULL); //process signal mask is unchanged since set is null pointer

        if((pid = fork()) < 0){                     //if forking fails (pid should be greater than 0)
            fprintf(stderr,"Forking Error\n");         //output stderr and error message
            return;
        }else if(pid == 0){                             //if child process (forked process is given pid == 0)
            setpgid(0, 0);                              //setting pgid to 0
            sigprocmask(SIG_UNBLOCK, &mask, NULL);      //process signal mask is unchanged since set is null pointer

            if(execvp(argv[0], argv) < 0) {             //executing command, if it fails print error
                fprintf(stderr,"%s: Command not found\n", argv[0]);
                exit(1);
            }
        }else{                                          //if is a builtin command
            if(!bg) addjob(jobs, pid, FG, cmdline);     //adds fg job to joblist if fg specified
            else addjob(jobs, pid, BG, cmdline);        //adds bg job to joblist if bg specified

            sigprocmask(SIG_UNBLOCK, &mask, NULL);      //signal blocking

            if (!bg) waitfg(pid);                      //if foreground process, wait for process to complete
            else printf("[%d] (%d) %s", pid2jid(pid), pid, cmdline);    //print jid, pid, and process/job
        }
    }
    return;
}
 ...
 *    it immediately.
 */
int find_IO(char **argv, char s[]){ //read input and determine if input or output redirect is called
    int count,i = 0;
...
int builtin_cmd(char **argv, char *cmdline)
{
    ...
    else if((find_IO(argv, ">") != 0) || (find_IO(argv, "<") != 0)){
    ...
    else if(find_IO(argv, "|") != 0){    //if piping is called
