...
int builtin_cmd(char **argv)
{
    ...
    if(!strcmp(argv[0], "fg")){ //executes do_bgfg
        do_bgfg(argv);
        return 1;
    }
    if(!strcmp(argv[0], "bg")){ //executes do_bgfg
        do_bgfg(argv);
        return 1;
    }
    return 0;
...
void do_bgfg(char **argv)
{
    ...
    if(id==NULL){                           //if the argument to bg or fg is null
        ...
        return;
    }else if(id[0] == '%' && is_num(id)){               //if the argument passed is a job id
        int jid = atoi(&id[1]);
    ...
    }else if(isdigit(id[0]) && is_num(id)){             //if the argument passed is a process id
        int pid = atoi(id);
